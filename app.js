(function () {
  ...
  function getLetterOptions(word, missingIndex) {
    ...
    var i;
    var letter;
    for (i = 0; options.length < 4 && i < letters.length; i += 1) {
  ...
  function updateHeader() {
    ...
    dom.soundToggle.innerHTML = saved.soundOn ? "Sound on" : "Sound off";
  ...
  function renderRound() {
    ...
    dom.targetWord.innerHTML = escapeHtml(displayWord(data.word));
  ...
  function titleForMode(mode) {
    ...
    return "Find the magic word";
  }

  function lineForMode(mode, word) {
    if (mode === "build") {
      return "Lumi found a word with one sleepy letter missing.";
    }
    if (mode === "sentence") {
      return "Read the sentence and pick the word that fits.";
    }
    if (mode === "story") {
      return "Lumi will glow brighter when you read the story word.";
    }
    if (mode === "choice") {
      return "Both paths are magical. Read each word, then choose.";
    }
    return "Tap the word that matches Lumi's glowing sign.";
  }

  function renderMatch(word) {
    var options = getChoiceEntries(word, 4);
    dom.challengeArea.innerHTML =
      '<p class="task-line">Can you tap <strong>' + escapeHtml(displayWord(word)) + '</strong>?</p>' +
      '<div class="word-options">' + optionButtons(options, word.key) + '</div>';
  }

  function renderBuild(word) {
    var text = displayWord(word).toLowerCase();
    var missing = Math.min(text.length - 1, (play.session.index + play.roundIndex) % text.length);
    var pattern = text.substring(0, missing) + "_" + text.substring(missing + 1);
    var options = getLetterOptions(text, missing);
    dom.challengeArea.innerHTML =
      '<p class="task-line">Which letter completes the word?</p>' +
      '<div class="sentence-line">' + escapeHtml(pattern) + '</div>' +
  ...
  function renderSentence(word) {
    var options = getChoiceEntries(word, 4);
    dom.challengeArea.innerHTML =
      '<p class="task-line">Pick the word that makes the sentence sparkle.</p>' +
      '<div class="sentence-line">' + escapeHtml(sentenceFor(word, true)) + '</div>' +
      '<div class="word-options">' + optionButtons(options, word.key) + '</div>';
  }

  function renderStory(word) {
    var visible = displayWord(word);
    dom.challengeArea.innerHTML =
      '<p class="story-text">Lumi tiptoes into a bright room. A little sign says <strong>' + escapeHtml(visible) + '</strong>. Lumi reads the word and the room begins to glow.</p>' +
      '<div class="word-options">' + optionButtons(getChoiceEntries(word, 4), word.key) + '</div>';
  }
  ...
  function renderChoice(word) {
    ...
    dom.challengeArea.innerHTML =
      '<p class="story-text">Lumi sees two glowing paths. Read both words. Which path should Lumi try?</p>' +
      '<div class="sentence-line">The path says: ' + escapeHtml(displayWord(word)) + ' or ' + escapeHtml(displayWord(other)) + '.</div>' +
  ...
  function renderComplete() {
    ...
    dom.targetWord.innerHTML = "Great reading";
    dom.challengeArea.innerHTML =
      '<p class="task-line">You finished today\'s word quest. Come back for the next glowing path.</p>' +
      '<div class="word-options"><button id="continueQuest" class="option-button" type="button">Next quest</button><button id="replayQuest" class="option-button" type="button">Play again</button></div>';
  ...
  function renderScene(currentWord) {
    ...
    var words = play.session ? play.session.words : [];
    var bg = sceneColors(tier);
    var labels = "";
    var i;
    var xPositions = [70, 220, 370, 520, 670];
    for (i = 0; i < words.length; i += 1) {
      labels += wordStone(xPositions[i], i === play.roundIndex && currentWord ? 266 : 286, displayWord(words[i]), i === play.roundIndex);
    }
    dom.sceneFrame.innerHTML =
      ...
      labels +
      lumiSvg(96, 286) +
      '<text x="150" y="388" font-size="18" font-family="Avenir Next, Verdana, sans-serif" font-weight="800" fill="#314f47">Lumi lights the way</text>' +
      '</svg>';
  }
  ...
  function wordStone(x, y, text, active) {
    var fill = active ? "#fff4ad" : "#ffffff";
    var stroke = active ? "#d89135" : "#609b83";
    var safe = escapeHtml(text);
    var size = text.length > 7 ? 19 : 22;
    return '<g>' +
      '<rect x="' + x + '" y="' + y + '" width="106" height="52" rx="8" fill="' + fill + '" stroke="' + stroke + '" stroke-width="4"/>' +
      '<text x="' + (x + 53) + '" y="' + (y + 34) + '" text-anchor="middle" font-size="' + size + '" font-family="Avenir Next, Verdana, sans-serif" font-weight="900" fill="#253346">' + safe + '</text>' +
      '</g>';
  }
